...
document.addEventListener('DOMContentLoaded', function() {
    const carousel = {
        track: document.getElementById('voiceCarouselTrack'),
        indicators: document.getElementById('voiceCarouselIndicators'),
        slides: null,
        currentSlide: 0,
        totalSlides: 0,
    ...
    window.addEventListener('resize', () => {
        ...
        resizeTimer = setTimeout(initCarouselIfNeeded, 250);
    });
});
// ================================
// 料金プランカルーセル機能（改良版）
...
document.addEventListener('DOMContentLoaded', function() {
    const priceCarousel = {
        track: document.getElementById('priceCarouselTrack'),
        indicators: document.getElementById('priceCarouselIndicators'),
        slides: null,
        currentSlide: 1, // 初期表示スライドのインデックス（0: ライト, 1: スタンダード, 2: プレミアム）
        totalSlides: 0,
        isTransitioning: false,
        touchStartX: 0,
        touchEndX: 0,
        ...
        init() {
            ...
            const originalSlides = Array.from(this.track.querySelectorAll('.carousel-slide'));
            ...
            this.track.insertBefore(lastClone, originalSlides[0]);
            
            this.slides = this.track.querySelectorAll('.carousel-slide');
            this.currentSlide = 1; // 初期表示をスタンダードプランに設定
            
            // トランジションを一時的に無効にして初期位置へ移動
            ...
            this.track.addEventListener('transitionend', () => {
                if (this.currentSlide === -1) { // 最後のクローンに到達した場合
                    this.track.style.transition = 'none';
                ...
                if (this.currentSlide === this.totalSlides) { // 最初のクローンに到達した場合
                    this.track.style.transition = 'none';
                    this.currentSlide = 0;
                    this.updateSlidePosition();
                    setTimeout(() => {
            ...
            this.setupTouchEvents();
            this.setupNavButtons(); // ナビゲーションボタンの設定を追加
        ...
        setupNavButtons() {
            const prevBtn = document.getElementById('pricePrevBtn');
            const nextBtn = document.getElementById('priceNextBtn');
        ...
        setupTouchEvents() {
            ...
            this.track.addEventListener('mousedown', (e) => this.touchStartX = e.clientX);
            this.track.addEventListener('mouseup', (e) => {
                this.touchEndX = e.clientX;
                this.handleSwipe();
            });
        ...
        updateSlidePosition() {
            ...
            // クローンを考慮した位置計算
            const translateX = offset - ((this.currentSlide + 1) * slideWidth);
            this.track.style.transform = `translateX(${translateX}px)`;
        },
        
        updateActiveClass() {
            this.slides.forEach(slide => slide.classList.remove('active'));
            const activeSlideIndex = (this.currentSlide + this.totalSlides) % this.totalSlides;
            if (this.slides[activeSlideIndex + 1]) {
                this.slides[activeSlideIndex + 1].classList.add('active');
            }
        },
        
        updateIndicators() {
            const indicatorElements = this.indicators.querySelectorAll('.indicator');
            const logicalIndex = (this.currentSlide + this.totalSlides) % this.totalSlides;
            indicatorElements.forEach((indicator, index) => {
    ...
    function initPriceCarouselIfNeeded() {
        if (window.innerWidth <= 768) {
            // 少し遅延を入れて初期化
            setTimeout(() => {
                priceCarousel.init();
    ...
    initPriceCarouselIfNeeded();
    
    let priceResizeTimer;
    window.addEventListener('resize', () => {
        clearTimeout(priceResizeTimer);
        priceResizeTimer = setTimeout(initPriceCarouselIfNeeded, 250);
    });
});
document.addEventListener('DOMContentLoaded', function() {
    // モバイルデバイスかどうかを判定する関数
